...
sortedInput : String -> List String
sortedInput str =
    str
        |> String.split " "
...
identifyNumbers : String -> Maybe (List String)
identifyNumbers str =
    let
        sorted =
            sortedInput str
        ...
        one =
            List.Extra.find (\s -> String.length s == 2) sorted

        four =
            List.Extra.find (\s -> String.length s == 4) sorted

        seven =
            List.Extra.find (\s -> String.length s == 3) sorted

        eight =
            List.Extra.find (\s -> String.length s == 7) sorted

        sixNineZero =
            List.filter (\s -> String.length s == 6) sorted

        twoThreeFive =
            List.filter (\s -> String.length s == 5) sorted

        middle =
            Maybe.map (\f -> f :: twoThreeFive) four
                |> Maybe.withDefault []
                |> findCommon
        ...
        zero =
            List.filter
                (\s ->
                    Maybe.map (\m -> String.contains m s |> not) middle |> Maybe.withDefault False
                )
                sixNineZero
                |> List.head
        ...
        sixNine =
            sixNineZero
                |> List.filter
                    (\s -> Maybe.map (\z -> z /= s) zero |> Maybe.withDefault False)

        nine =
            getNine one sixNine

        six =
            List.Extra.find (\s -> Maybe.map (\n -> n /= s) nine |> Maybe.withDefault False) sixNine

        five =
            Maybe.Extra.andThen3 getFive six nine eight

        twoThree =
            List.filter (\s -> Maybe.map (\f -> f /= s) five |> Maybe.withDefault False) twoThreeFive

        three =
            getThree one twoThree

        two =
            getTwo three twoThree
    in
    [ zero, one, two, three, four, five, six, seven, eight, nine ] |> Maybe.Extra.combine


getThree : Maybe String -> List String -> Maybe String
getThree maybeOne list =
    Maybe.andThen
        (\one ->
            List.Extra.find (\n -> List.Extra.isSubsequenceOf (String.toList one) (String.toList n)) list
        )
        maybeOne


getTwo : Maybe String -> List String -> Maybe String
getTwo maybeThree list =
    Maybe.andThen
        (\three ->
            List.Extra.find
                (\s -> s /= three)
...
findCommon : List String -> Maybe String
findCommon ls =
    ls
        |> List.concatMap String.toList
        |> List.Extra.gatherEquals
        |> List.Extra.find (\( _, list ) -> List.length list == List.length ls - 1)
        |> Maybe.map (Tuple.first >> String.fromChar)


getNine : Maybe String -> List String -> Maybe String
getNine maybeOne list =
    Maybe.andThen
        (\one ->
            list
                |> List.filter (\s -> List.Extra.isSubsequenceOf (String.toList one) (String.toList s))
                |> List.head
        )
        maybeOne


getFive : String -> String -> String -> Maybe String
getFive six nine eight =
    "abcdefg"
        |> String.split ""
        |> List.filter (\str -> List.all (\a -> String.contains str a) [ six, nine, eight ])
        |> String.join ""
        >> Just


getNumber : List String -> String -> Maybe Int
getNumber dictList str =
    let
        sortedStr =
            String.toList str |> List.sort |> String.fromList
    in
    List.Extra.findIndex ((==) sortedStr) dictList


getNumberFromReading : ( String, String ) -> Maybe Int
getNumberFromReading ( dictString, str ) =
    str
        |> String.split " "
        |> List.indexedMap
            (\idx s ->
                let
                    maybeNumber =
                        Maybe.andThen (\d -> getNumber d s) (identifyNumbers dictString)
                in
                Maybe.map ((*) (getPlaceMultiplier idx)) maybeNumber
            )
        |> Maybe.Extra.combine
        |> Maybe.map (List.foldl (+) 0)


getPlaceMultiplier int =
    if int == 0 then
        1000
...
test =
    getNumberFromReading ( "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab", "cdfeb fcadb cdfeb cdbaf" )
...
formatInput inp =
    inp |> String.lines |> List.map (String.split " | ")
...
convertInputToTuples : String -> Maybe (List ( String, String ))
convertInputToTuples inp =
    inp
        |> formatInput
...
run2Test =
    testInput
        |> convertInputToTuples
        |> Maybe.andThen
            (List.map getNumberFromReading >> Maybe.Extra.combine)
        |> Maybe.map (List.foldl (+) 0)
